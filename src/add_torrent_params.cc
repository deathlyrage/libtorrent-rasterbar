...
namespace libtorrent_rasterbar {
	...
	int ObjectToAddTorrentParams(v8::Local<v8::Object> obj, libtorrent::add_torrent_params& p)
	{
		...
		if (HAS_KEY(obj, "ti")) {
			...
			p.ti = Nan::ObjectWrap::Unwrap<TorrentInfo>(Nan::To<v8::Object>(ti).ToLocalChecked())->ti;
		}
		...
		if (HAS_KEY(obj, "trackers")) {
			...
			for (uint32_t i = 0, length = v8::Local<v8::Array>::Cast(trackers)->Length(); i < length; ++i) {
				GET_WITH_CHECK(Nan::To<v8::Object>(trackers).ToLocalChecked(), i, tracker, IsString, ("trackers." + std::to_string(i)).c_str(), "String");
		...
		if (HAS_KEY(obj, "urlSeeds")) {
			...
			for (uint32_t i = 0, length = v8::Local<v8::Array>::Cast(urls)->Length(); i < length; ++i) {
				GET_WITH_CHECK(Nan::To<v8::Object>(urls).ToLocalChecked(), i, url, IsString, ("urlSeeds." + std::to_string(i)).c_str(), "String");
		...
		if (HAS_KEY(obj, "dhtNodes")) {
			...
			for (uint32_t i = 0, length = v8::Local<v8::Array>::Cast(nodes)->Length(); i < length; ++i) {
				GET_WITH_CHECK(Nan::To<v8::Object>(nodes).ToLocalChecked(), i, node, IsObject, ("dhtNodes." + std::to_string(i)).c_str(), "Object");
				GET_KEY_WITH_CHECK(Nan::To<v8::Object>(node).ToLocalChecked(), "host", host, IsString, ("dhtNodes." + std::to_string(i) + ".host").c_str(), "String");
				GET_KEY_WITH_CHECK(Nan::To<v8::Object>(node).ToLocalChecked(), "port", port, IsNumber, ("dhtNodes." + std::to_string(i) + ".port").c_str(), "Number");
				p.dht_nodes.push_back(std::make_pair(std::string(*Nan::Utf8String(host)), (int)port->Int32Value(Nan::GetCurrentContext()).FromJust()));
			}
		}
		...
		if (HAS_KEY(obj, "storageMode")) {
			...
			p.storage_mode = (libtorrent::storage_mode_t) storage_mode->Int32Value(Nan::GetCurrentContext()).FromJust();
		}
		...
		if (HAS_KEY(obj, "filePriorities")) {
			...
			for (uint32_t i = 0, length = v8::Local<v8::Array>::Cast(priorities)->Length(); i < length; ++i) {
				GET_WITH_CHECK(Nan::To<v8::Object>(priorities).ToLocalChecked(), i, priority, IsNumber, ("filePriorities." + std::to_string(i)).c_str(), "Number");
				p.file_priorities.push_back((boost::uint8_t) priority->Int32Value(Nan::GetCurrentContext()).FromJust());
			}
		}
		...
		if (HAS_KEY(obj, "flags")) {
			...
			p.flags = (boost::uint64_t) flags->Int32Value(Nan::GetCurrentContext()).FromJust();
		}
		...
		if (HAS_KEY(obj, "maxUploads")) {
			...
			p.max_uploads = (boost::uint64_t) value->Int32Value(Nan::GetCurrentContext()).FromJust();
		}

		if (HAS_KEY(obj, "maxConnections")) {
			GET_KEY_WITH_CHECK(obj, "maxConnections", value, IsNumber, "maxConnections", "Number");
			p.max_connections = (boost::uint64_t) value->Int32Value(Nan::GetCurrentContext()).FromJust();
		}

		if (HAS_KEY(obj, "uploadLimit")) {
			GET_KEY_WITH_CHECK(obj, "uploadLimit", value, IsNumber, "uploadLimit", "Number");
			p.upload_limit = (boost::uint64_t) value->Int32Value(Nan::GetCurrentContext()).FromJust();
		}

		if (HAS_KEY(obj, "downloadLimit")) {
			GET_KEY_WITH_CHECK(obj, "downloadLimit", value, IsNumber, "downloadLimit", "Number");
			p.download_limit = (boost::uint64_t) value->Int32Value(Nan::GetCurrentContext()).FromJust();
		}
